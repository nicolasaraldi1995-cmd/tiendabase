...
<template>
    <Head :title="`Marca ${marca.nombre}`" />
    <PublicLayout>
        <div class="px-6 py-8">
            <div class="flex items-center gap-3 mb-6">
                <span class="w-8 h-8 rounded-xl bg-accent/10 flex items-center justify-center text-accent font-bold text-sm">
                    {{ marca.nombre.charAt(0).toUpperCase() }}
                </span>
                <h1 class="text-xl font-semibold text-text">Marca {{ marca.nombre }}</h1>
            ...
            <div v-else class="text-center py-20 text-text-muted">Todavía no hay productos de {{ marca.nombre }}.</div>
