...
<title>{{ $negocio->nombre_negocio }} · Lista de precios {{ $generado->format('d/m/Y') }}</title>
<style>
    :root{
        --papel:#f3f1ec;
        --tinta:#1a1d21;
...
<div class="wrap">
    ...
    <header>
        @if($logoData)<img src="{{ $logoData }}" alt="{{ $negocio->nombre_negocio }}">@endif
        <div>
            <div class="marca-nombre">{{ $negocio->nombre_negocio }}</div>
            @if($negocio->eslogan)<div class="marca-sub">{{ $negocio->eslogan }}</div>@endif
        </div>
...
(function () {
    ...
    var GUARDADO = 'tienda-pedido';
    var GUARDADO_NEGOCIO = 'tienda-negocio';
