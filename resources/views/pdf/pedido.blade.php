...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { display: flex; justify-content: space-between; margin-bottom: 25px; border-bottom: 2px solid {{ $negocio->colorAcento() }}; padding-bottom: 15px; }
        .brand { font-size: 20px; font-weight: bold; color: {{ $negocio->colorAcento() }}; }
        .brand small { display: block; font-size: 10px; color: #9a9da5; font-weight: normal; }
        .info { text-align: right; }
        .info p { margin: 2px 0; }
        .section { margin: 20px 0; }
        .section h3 { font-size: 13px; color: {{ $negocio->colorAcento() }}; margin-bottom: 8px; border-bottom: 1px solid #e6e4df; padding-bottom: 4px; }
        ...
        .badge { display: inline-block; background: {{ $negocio->colorAcento() }}; color: #fff; padding: 2px 10px; border-radius: 10px; font-size: 10px; }
...
<body>
    @php($logoData = $negocio->logoDataUri())
    @if($logoData)
    <div class="watermark">
    ...
    <div class="header">
        <div>
            <div class="brand">{{ $negocio->nombre_negocio }}@if($negocio->eslogan)<small>{{ $negocio->eslogan }}</small>@endif</div>
        </div>
        <div class="info">
    ...
    @php($verPrecios = auth()->id() === $pedido->user_id || (auth()->user()?->isAdmin() ?? false))
    ...
    <div class="footer">
        {{ $negocio->nombre_negocio }}@if($negocio->eslogan) — {{ $negocio->eslogan }}@endif @if($negocio->ciudad) · {{ $negocio->ciudad }}@endif
    </div>
</body>
</html>
