...
class SincronizarCatalogo
{
     ...
     */
    public function analizar(string $archivo, int $filaEncabezados = 1): array
    {
        $vacio = ['cambiosDeMarca' => [], 'cambiosDeNombre' => [], 'bajas' => [], 'dudosos' => [], 'sinCambios' => 0];

        $filas = $this->importador->readFile($archivo, $filaEncabezados);

        // Las columnas no se pueden dar por sentadas: la planilla que se le
        // entrega al negocio las escribe en minúscula y la lista que exporta la
        // web en mayúscula. Si no coinciden, acá no se leería ninguna fila y
        // todo el catálogo terminaría clasificado como baja.
        $primera = $filas->first() ?? [];
        $colNombre = $this->columna($primera, ['nombre', 'producto', 'name', 'descripcion']);
        $colMarca = $this->columna($primera, ['marca', 'brand']);

        // Sin esas dos columnas no hay con qué comparar. Devolver un plan vacío
        // es lo correcto: dar de baja el catálogo porque no se entendió el
        // archivo sería el peor error posible.
        if ($colNombre === null || $colMarca === null) {
            return $vacio;
        }

        $filas = $filas->filter(fn ($f) => ! empty($f[$colNombre]) && ! empty($f[$colMarca]));
     ...
     * (un mismo producto podía estar cargado bajo dos marcas distintas, y la
     * lista los manda a los dos a la misma). Se conserva el que tiene foto -- y
    ...
    /**
     * Con qué clave viene una columna en este archivo, sin importar cómo esté
     * escrita. Null si no está.
     *
     * @param  array<string, mixed>  $fila
     * @param  list<string>  $candidatos
     */
    private function columna(array $fila, array $candidatos): ?string
    {
        foreach (array_keys($fila) as $clave) {
            if (in_array(mb_strtolower(trim((string) $clave)), $candidatos, true)) {
                return $clave;
            }
        }
