            ...
            Presentacion::withTrashed()
                ...
                ->chunkById(200, function ($presentaciones) use ($conIva, &$stats) {
                    foreach ($presentaciones as $presentacion) {
                        ...
                        // El precio por mayor se compara de igual a igual contra
                        // el de lista (ver Presentacion::precioPara), así que
                        // tiene que llevar el mismo IVA. Sin esto, prender la
                        // marca dejaba a todos los clientes mayoristas comprando
                        // sin IVA en silencio, y apagarla dejaba el mayorista más
                        // caro que el precio normal. Se ajusta con el factor aun
                        // cuando el precio se rehizo desde el costo: el mayorista
                        // no sale del margen, es un número que se tipea.
                        if ($presentacion->precio_mayorista !== null) {
                            $presentacion->precio_mayorista = round(
                                $conIva
                                    ? (float) $presentacion->precio_mayorista * self::FACTOR
                                    : (float) $presentacion->precio_mayorista / self::FACTOR,
                                2
                            );
                        }
